...
"""def _extraer_fecha_expedicion(texto: str) -> str:
    ...
    m = re.search(r"FECHA\s*(DE\s*)?EXPEDICI[ÓO]N\s*:?\s*(\d{2}/\d{2}/\d{4})", texto, flags=re.IGNORECASE)
    if m:
        print(f"    [DEBUG] Caso 1 encontrado: {m.group(2)}")
        return m.group(2)

    m = re.search(r"EXPEDICI[ÓO]N\s*:?\s*(\d{2}/\d{2}/\d{4})", texto, flags=re.IGNORECASE)
    if m:
    ...
    for i, linea in enumerate(lineas):
        if "EXPEDICI" in linea.upper() and "LUGAR" not in linea.upper():
            print(f"    [DEBUG] Línea con EXPEDICI encontrada: '{linea}'")
            if i + 1 < len(lineas):
                print(f"    [DEBUG] Siguiente línea: '{lineas[i+1]}'")
                m = re.search(r"(\d{2}/\d{2}/\d{4})", lineas[i+1])
                if m:
                    print(f"    [DEBUG] Caso 4 encontrado: {m.group(1)}")
                    return m.group(1)

    m = re.search(r"(EXPEDICI[ÓO]N|EMISI[ÓO]N|EMITIDO|FECHA\s+DE\s+EMISI[ÓO]N).*?(\d{2}/\d{2}/\d{4})", texto, flags=re.IGNORECASE)
    if m:
